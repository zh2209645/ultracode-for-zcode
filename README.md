...
Current release: **v0.2.0** (published from the signed `v0.2.0` tag on 2026-08-27; post-release comprehensive verification passed on ZCode 3.9.2 the same day, docs/08 §21).
...
The behavioral regression was executed on v0.1.3 (ZCode 3.7.7). Release v0.1.4 is a declarative documentation and release-integrity update with unchanged Skill, Command, and Agent behavior; it closed the final S-12 gate by synchronizing all version fields, pinning the marketplace to `v0.1.4`, and publishing a valid GPG-signed tag. Release v0.1.5 ships the request-sizing, context-hygiene, and orchestrator-mode prompt changes plus the English policy documents; the mandatory regression including the new F-15 scenario was re-run on v0.1.5 and passed in full — nine scenarios plus three fix-seam probes (2026-08-19, docs/08 §14). After the host updated to ZCode 3.8.1 (2026-08-19), core functionality — plugin loading, command/skill mounting, agent registration, and live Explore/worker-fast/worker-review dispatch — was re-verified in a fresh plugin session (docs/08 §15). On 2026-08-23 the same core re-verification passed on WSL2 (Ubuntu) — the first non-Windows host record, with the installed cache verified byte-identical to the signed v0.1.5 tag (docs/08 §16). On 2026-08-25, after the host updated to ZCode 3.9.1, the core re-verification passed again in a fresh /ultracode orchestrator session — this time with all four worker tiers (fast/standard/deep/review) dispatched live — and the official changelog confirmed no plugin/skill/agent/subagent/marketplace surface changes between 3.8.1 and 3.9.1 (docs/08 §17). On 2026-08-27, release v0.2.0 (five agents incl. `explore-flash`) passed its post-release comprehensive verification in a fresh ZCode 3.9.2 `/ultracode` session: install chain closed end-to-end, all five agents live-dispatched, nine gates plus fix-seam probes (i)/(ii) live and (iii) executed as the record write itself, routing 15/15, zero nested delegation or retries; the `explore-flash`→Explore escalation path stayed policy-verified only (trigger never arose) (docs/08 §21).
