...
Model IDs are pre-configured for the local machine
(`builtin:zai-coding-plan/glm-5.3-flash` for explore-flash/fast/standard and
`builtin:zai-coding-plan/GLM-5.3` for deep/review — see `MODEL-MAPPING.md`). Release `v0.2.0` is published as
...
4. Start a new session; the command, skill, the read-only explorer, three write workers, and the read-only reviewer become available.
...
- The three write workers declare `Read`, `Edit`, `Write`, `Glob`, and `Grep`; `worker-review` and `explore-flash` declare only `Read`, `Glob`, and `Grep`. All exclude `Bash`, `WebFetch`, `WebSearch`, and session MCP tools. Command execution, network research, and test/build verification stay with the primary Agent.
...
- `maxTurns` bounds all agents (12/24/36 for the three write workers, 30 for `worker-review`, and 16 for `explore-flash`); subagents cannot spawn subagents; the primary Agent re-verifies evidence before accepting `done`.
- High-assurance independent review and adjudication use `worker-review`, which runs the configured high-performance model at maximum reasoning depth. Low-to-medium difficulty exploration, routine investigation, and evidence collection use `explore-flash`; high-difficulty or broad exploration uses built-in `Explore`; ordinary analysis stays with the primary Agent. Only high-risk or high-complexity tasks that explicitly require file changes route to `worker-deep`.
- At most 3 write-capable workers run concurrently. Genuinely independent read-only `explore-flash`, Explore, or `worker-review` tasks may raise total concurrency to 10.
- Workspace `.zcode/**` is available only to the three write workers when the delegated task explicitly writes files, and only for task-owned todo/log records. `explore-flash`, Explore, and `worker-review` report through the ZCode subagent result; `worker-review` and `explore-flash` cannot write files and must not read `.zcode`. User-level `~/.zcode`, plugin caches, credentials, unrelated session logs, and plugin-owned recovery databases remain out of scope. Shared `.zcode` files are updated serially; parallel write workers use distinct log files.
...
- `marketplace.json` is pinned to the signed `v0.2.0` release tag. Protect or sign release tags, and bump the marketplace, plugin manifest, Skill metadata, and tag together for every release.
