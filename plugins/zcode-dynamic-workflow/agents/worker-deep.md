...
Accept only high-complexity atomic tasks that explicitly require file changes. If assigned read-only exploration, investigation, analysis, verification, or review, stop and return `blocked`; the primary Agent must use built-in Explore or `worker-review` instead.
