...
description: Use for substantive coding, refactoring, investigation, review, migration, or multi-part work where the primary Agent should decide whether to work directly or delegate atomic tasks to ZCode Explore, explore-flash, worker-review, worker-fast, worker-standard, or worker-deep based on task type, difficulty, risk, dependencies, and verification cost.
...
Use the plugin's `explore-flash` for low-to-medium difficulty read-only research — targeted search, routine investigation, implementation lookup, and evidence gathering — at lower cost; use ZCode's built-in `Explore` for high-difficulty or broad exploration (deep call-chain mapping, cross-module discovery, very thorough sweeps) and when `explore-flash` evidence is insufficient. Do not bulk-read files in the primary context when a targeted exploration result (paths, line numbers, short quotes) is enough; ordinary analysis stays with the primary Agent but works from that targeted evidence.
...
6. Run at most 3 write-capable workers concurrently. Read-only `explore-flash`, built-in `Explore`, and `worker-review` tasks may raise the total concurrent subagent count to 10 when they are genuinely independent; 10 is a ceiling, not a target.
...
- Read-only codebase research -> `explore-flash` (glm-5.3-flash, `thoughtLevel: low`, lower cost) for low-to-medium difficulty targeted search, discovery, routine investigation, and evidence gathering; -> built-in `Explore` for high-difficulty or broad research (deep call-chain mapping, cross-module discovery, very thorough sweeps) and when `explore-flash` evidence is insufficient. Ordinary analysis that does not require an independent review verdict stays with the primary Agent.
...
Launch independent tasks together, up to 3 write-capable workers. Additional concurrent tasks up to the total limit of 10 must use read-only `explore-flash`, built-in `Explore`, or `worker-review`.
...
- Unknown root cause, unclear scope, or missing evidence -> `explore-flash` (routine discovery), built-in `Explore` (high-difficulty discovery), or primary-Agent re-planning; do not escalate to a write worker. An `explore-flash` result that is partial or insufficient may escalate once to built-in `Explore`.
...
Built-in `Explore` may return prose instead of this exact format, but its conclusions must still carry file, line, or command evidence. `explore-flash` returns the structured block with `EVIDENCE` in place of `FILES / EVIDENCE` and never reports changed files.
...
- which tasks were direct, explore-flash, Explore, review, fast, standard, or deep;
